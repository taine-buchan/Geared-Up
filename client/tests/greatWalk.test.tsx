...
describe('Visiting the great walk page', () => {
  ...
  it('shows an error message when server failed', async () => {
    const testId = 1
    nock('http://localhost').get(`/api/v1/great-walks/${testId}`).reply(500, 'Error!')
    const screen = renderRoute(`/great-walks/${testId}`)
  ...
  it('shows a list of great walks', async () => {
    ...
    nock('http://localhost').get(`/api/v1/great-walks/${testGreatWalk.id}`).reply(200, testGreatWalk)
    const screen = renderRoute(`/great-walks/${testGreatWalk.id}`)

    const listHeading = await screen.findByRole('heading', {name: 'Required Equipment'})
